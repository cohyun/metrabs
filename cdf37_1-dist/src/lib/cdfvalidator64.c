...
static CDFstatus QuitCDF PROTOARGs((char *, OFF_T, int, int, void *, void *,
                                    Logical));
static CDFstatus ValidateCCR PROTOARGs((vFILE *, OFF_T, Logical));
static CDFstatus ValidateCDR PROTOARGs((vFILE *, OFF_T, Logical));
static CDFstatus ValidateGDR PROTOARGs((vFILE *, OFF_T, Logical));
static CDFstatus ValidateVDR PROTOARGs((struct CDFstruct *, vFILE *, OFF_T,
                                        Logical, Logical));
static CDFstatus ValidateVXR PROTOARGs((vFILE *, OFF_T, Int32, Logical));
static CDFstatus ValidateVVR PROTOARGs((vFILE *, OFF_T, Logical));
static CDFstatus ValidateADR PROTOARGs((struct CDFstruct *, vFILE *, OFF_T,
                                        Logical));
static CDFstatus ValidateAEDR PROTOARGs((struct CDFstruct *, vFILE *, OFF_T,
                                         Int32, Int32, Logical, Logical));
static CDFstatus ValidateCPR PROTOARGs((vFILE *, OFF_T, Logical));
static CDFstatus ValidateSPR PROTOARGs((vFILE *, OFF_T, Logical));
static CDFstatus ValidateCVVR PROTOARGs((vFILE *, OFF_T, Logical));
static CDFstatus ValidateUIR PROTOARGs((vFILE *, OFF_T, Logical));
static CDFstatus ValidateAttributeLinks PROTOARGs((struct CDFstruct *, vFILE *,
                                                   Logical));
static CDFstatus ValidateAttributeEntryLink PROTOARGs((struct CDFstruct *,
                                                       vFILE *, Int32, Logical,
                                                       OFF_T, Int32, Int32,
                                                       Logical));
static CDFstatus ValidateVariableLinks PROTOARGs((struct CDFstruct *, vFILE *,
                                                  Logical, Logical));
static CDFstatus ValidateVariableValueLinks PROTOARGs((struct CDFstruct *,
                                                       vFILE *, Int32, OFF_T,
                                                       Logical));
...
#if !defined(win32) && !defined(__MINGW32__)
          snprintf (text2, (size_t) sizeof(text2),
		    "(%lld vs %lld)", *(long long *)value1, 
                                      *(long long *)value2);
#else
          snprintf (text2, (size_t) sizeof(text2),
		    "(%I64d vs %I64d)", *(long long *)value1,
                                        *(long long *)value2);
#endif
          strcatX (text, text2, 150);
...
static CDFstatus ValidateGDR (vFILE *fp, OFF_T offset, Logical debug)
{
  ...
  if (GDR.NumAttr < 0 || (GDR.NumAttr > 0 && GDR.ADRhead < 1)) 
    return QuitCDF ("CDF(GDR): # of attributes or their link is invalid ", offset,
  ...
  if (GDR.rNumDims < 0 || GDR.rNumDims > CDF_MAX_DIMS) 
    return QuitCDF ("CDF(GDR): # of dimensions for rVars is invalid ", offset,
                              ...
                              Logical debug)
{
  ...
  if ((ADR.Scope == GLOBAL_SCOPE) && (ADR.NzEntries > 0))
    return QuitCDF ("CDF(ADR): global attribute zEntry is invalid ", offset,
                    4, 1, &(ADR.NzEntries), 0, debug);
  if ((ADR.Scope == GLOBAL_SCOPE) && (ADR.AzEDRhead > 0))
    return QuitCDF ("CDF(ADR): global attribute zEntry link is invalid ", offset,
...
static CDFstatus ValidateVXR (vFILE *fp, OFF_T offset, Int32 lastRec,
                              Logical debug)
{
  ...
  if (VXR.VXRnext > 0) {
    if (VXR.Last[VXR.NusedEntries-1] > lastRec)
      return QuitCDF ("CDF(VXR): a variable last record does not match in a Variable Index Record: ",
                      offset, 4, 2, &(VXR.Last[VXR.NusedEntries-1]), &lastRec,
                      debug);
    status = ValidateVXR (fp, VXR.VXRnext, lastRec, debug);
    if (status != CDF_OK) return status;
  } else if (VXR.VXRnext < 0) {
      return QuitCDF ("CDF(VXR): a link offset to next record is negative in a Variable Index Record: ",
                      offset, 8, 1, &(VXR.VXRnext), 0, debug);
  }

  for (i = 0; i < VXR.NusedEntries; ++i) {
    if (VXR.First[i] < 0 || VXR.Last[i] < 0 || VXR.First[i] > VXR.Last[i])
      return QuitCDF ("CDF(VXR): entry value for first/last is invalid ",
                      offset, 4, 2, &(VXR.First[i]), &(VXR.Last[i]), debug);
    if (VXR.Offset[i] < 1) 
                                         ...
                                         Logical debug)
{
  ...
  visit = (int *) cdf_AllocateMemory ((size_t)GDR.NumAttr * sizeof(Int32),
                                      NULL);
                                             ...
                                             Logical debug)
{
  ...
  offset = EDRhead;
  lastNum = 0;
  visits = (Int32 *) cdf_AllocateMemory ((size_t)numEntries * sizeof(Int32),
                                         NULL);
    ...
    if (ix > 0) {
      for (iy = 0; iy < ix; ++iy) {
        if (visits[iy] == entryNum) {
          cdf_FreeMemory (visits, NULL);
          return QuitCDF ("CDF: entry # is repeating in an attr entry link: ",
                          (OFF_T) -1, 4, 1, &entryNum, 0, debug);
        }
      }
    }
    if (ix == (int) numEntries) {
      cdf_FreeMemory (visits, NULL);
      return QuitCDF ("CDF: # of entries is more than max in an attr link: ",
                      (OFF_T) -1, 4, 1, &ix, 0, debug);
    }
    visits[ix] = entryNum;
    ++ix;
  ...
  if (lastNum != maxEntry) {
    cdf_FreeMemory (visits, NULL);
    return QuitCDF ("CDF: last entry # is not the max entry # in an attr link: ",
                    (OFF_T) -1, 4, 2, &lastNum, &maxEntry, debug);
  }
  cdf_FreeMemory (visits, NULL);
  return CDF_OK;
}
...
{
  CDFstatus status;
  OFF_T offset, headVXR, tailVXR, nextVDR, cprOffset;
  ...
  for (ix = 0; ix < numVars; ++ix) {
    if (visit[ix] == 0) {
      cdf_FreeMemory (visit, NULL);
      if (zVar)
        return QuitCDF ("CDF: a zVar unreachable in the variable chain: ",
                        (OFF_T) -1, 4, 1, &ix, 0, debug);
      else
        return QuitCDF ("CDF: a rVar unreachable in the variable chain: ",
                        (OFF_T) -1, 4, 1, &ix, 0, debug);
    }
  }
  cdf_FreeMemory (visit, NULL);
  if (offset != 0) {
    ...
    if (status == CDF_OK) {
      if (num >= numVars) { 
        if (zVar)
          return QuitCDF ("CDF(VDR): a zVar unreachable in the variable chain: ",
                          offset, 4, 1, &num, 0, debug);
        else
          return QuitCDF ("CDF(VDR): a rVar unreachable in the variable chain: ",
                          offset, 4, 1, &num, 0, debug);
      } else {
        if (zVar) 
          return QuitCDF ("CDF(VDR): a zVar is repeated in the variable chain: ",
                          offset, 4, 1, &num, 0, debug);
        else
          return QuitCDF ("CDF(VDR): a rVar unreachable in the variable chain: ",
                          offset, 4, 1, &num, 0, debug);
