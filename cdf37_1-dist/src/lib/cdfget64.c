...
*                               routine.
*   Note: Those offset fields for CURgrEntryOffset64, CURzEntryOffset64, 
*         CURattrOffset64, etc, in CDF struture may not have the proper 
*         values set if the file is open with READONLY mode. Their actual
*         offsets are kept in a sepate buffers.
******************************************************************************/
...
/*  if (!ValidCDFname(CDFpathT)) return BAD_CDF_NAME; */
  ...
  ****************************************************************************/
  ...
  case zVAR_PADVALUE_: {
    ...
    ***********************************************************************/
    if (STRINGdataType(dataType)) numElems = 1;
  ...
  ****************************************************************************/
  ...
  /****************************************************************************
  * rENTRY_NUMSTRINGS_/zENTRY_NUMSTRINGS_
  ****************************************************************************/
  case rENTRY_NUMSTRINGS_:
  case zENTRY_NUMSTRINGS_: {
    Logical dataOp = (Va->item == rENTRY_NUMSTRINGS_);
    struct CDFstruct *CDF;
    long *value = va_arg (Va->ap, long *);
    Int32 tValue;
    OFF_T eOffset;
    SelectCDF (Cur->cdf, CDF)
    if (!CURRENTattrSELECTED64(CDF)) return NO_ATTR_SELECTED;
    if (BOO(dataOp, CDF->CURgrEntryNum, CDF->CURzEntryNum) == 
        RESERVED_ENTRYNUM) return NO_ENTRY_SELECTED;
    if (!sX(CheckEntryOp64(CDF,Va->item),&pStatus)) return pStatus;
    eOffset = BOO(dataOp, CDF->CURgrEntryOffset64,
			  CDF->CURzEntryOffset64);
    if (eOffset == (OFF_T) RESERVED_ENTRYOFFSET64) return NO_SUCH_ENTRY;
    if (!sX(ReadAEDR64(CDF->fp,eOffset,
		       AEDR_NUMSTRINGS,&tValue,
		       AEDR_NULL),&pStatus)) {
      AbortAccess64 (CDF, UPDATE, noDELETE);
      return pStatus;
    }
    *value = (long) BOO((tValue>0), tValue, 1);
    break;
  }
  /****************************************************************************
  ...
  ****************************************************************************/
  ...
  case zENTRY_STRINGSDATA_: {
    ...
    char ***strings = va_arg (Va->ap, char ***);
    char *tmp;
    SelectCDF (Cur->cdf, CDF)
    if (!CURRENTattrSELECTED64(CDF)) return NO_ATTR_SELECTED;
    if ((!zOp?CDF->CURgrEntryNum:CDF->CURzEntryNum) == RESERVED_ENTRYNUM)
      return NO_ENTRY_SELECTED;
    offset = (!zOp?CDF->CURgrEntryOffset64:CDF->CURzEntryOffset64);
    if (offset == (OFF_T) RESERVED_ENTRYOFFSET64) return NO_SUCH_ENTRY;
    if (!sX(ReadAEDR64(CDF->fp,offset,
		       AEDR_DATATYPE,&dataType,
		       AEDR_NUMELEMS,&numElems,
    ...
    if (!STRINGdataType(dataType)) return TRY_TO_READ_NONSTRING_DATA;
    tmp = (char *) malloc((size_t)numElems+1);
    if (!sX(ReadAEDR64(CDF->fp,offset,
		       AEDR_NUMSTRINGS,numStrings,
		       AEDR_VALUE,tmp,
		       AEDR_NULL),&pStatus)) {
      AbortAccess64 (CDF, UPDATE, noDELETE);
      return pStatus;
    ...
    *(tmp+numElems) = '\0';
    /* Pre-3.7.0, the field was used and filled with 0. */ 
    if (*numStrings < 1) *numStrings = 1;
    StringBreakIntoStrings (tmp, numElems, *numStrings, strings);
    free (tmp);
    break;
  }
  /****************************************************************************
