...
switch (Va->item) {
  ...
  ****************************************************************************/
  ...
  case zVAR_COMPRESSION_: {
    ...
    Int32 maxAllocated, pCount = 0; int p; OFF_T VDRoffset;
    struct CDFstruct *CDF; struct VarStruct *Var;
    struct VDRstruct64 VDR; struct CPRstruct64 CPR;
    Int32 numDims, dimSizes[CDF_MAX_DIMS], recSize;
    int i;
    long cType = va_arg (Va->ap, long);         /* Compression type. */
    long *cParms = va_arg (Va->ap, long *);     /* Compression parameters. */
    /**************************************************************************
    ...
    **************************************************************************/
    ...
    /**************************************************************************
    * If the existing blocking factor is too small, then give warning.
    **************************************************************************/
    if (!zOp) {
      if (!sX(ReadGDR64(CDF->fp,CDF->GDRoffset64,
			GDR_rNUMDIMS,&numDims,
			GDR_rDIMSIZES,dimSizes,
			GDR_NULL),&pStatus)) {
	AbortAccess64 (CDF, UPDATE, noDELETE);
	return pStatus;
      }
    }
    if (RECvaryBITset(VDR.Flags) && (VDR.blockingFactor != 0)) {
      recSize = VDR.NumElems;
      recSize *= (Int32) CDFelemSize(VDR.DataType);
      for (i = 0; i < (int) (zOp?VDR.zNumDims:numDims); ++i) 
	if (VDR.DimVarys[i]) recSize *= (zOp?VDR.zDimSizes[i]:dimSizes[i]);
      if (VDR.blockingFactor < 
	  (Int32) (MIN_BLOCKING_BYTES_compressed/recSize)) {
	sX (BLOCKINGFACTOR_TOO_SMALL, &pStatus);
/*	return pStatus; */
      }
    }
    /**************************************************************************
  ...
  ****************************************************************************/
  ...
  case zENTRY_DATA_: {
	...
	********************************************************************/
	...
	AEDR.NumElems = numElems;
        if ((Va->item == rENTRY_DATA_ || Va->item == zENTRY_DATA_) &&
            STRINGdataType(dataType)) {
          long numStrings = FindNumberOfItems ((char *)value, numElems,
                                               STRINGDELIMITER);
          AEDR.NumStrings = numStrings;
        } else
          AEDR.NumStrings = 0;
  ...
  /****************************************************************************
  * rENTRY_STRINGSDATA_/zENTRY_STRINGSDATA_, 
  ****************************************************************************/
  case rENTRY_STRINGSDATA_:
  case zENTRY_STRINGSDATA_: {
    Logical zOp = (Va->item == zENTRY_STRINGSDATA_);
    struct CDFstruct *CDF;
    struct ADRstruct64 ADR;
    long entryN;                /* True entry number. */
    Logical zEntry;             /* If true, a AzEDR.  If FALSE, a AgrEDR. */
    ...
    OFF_T eOffset;              /* Offset of AEDR. */
    int ix, length; char *tmpStr, *cstr;
    long numStrings = va_arg (Va->ap, long);
    char **values = va_arg (Va->ap, char **);
    SelectCDF (Cur->cdf, CDF)
    if (!CURRENTattrSELECTED64(CDF)) return NO_ATTR_SELECTED;
    if ((!zOp?CDF->CURgrEntryNum:CDF->CURzEntryNum) == RESERVED_ENTRYNUM)
      return NO_ENTRY_SELECTED;
    if (numStrings < 1) return BAD_NUM_STRINGS;
    length = 0;
    for (ix = 0; ix < numStrings; ++ix) {
      length += (int) strlen(values[ix]);
    }
    dataType = CDF_CHAR;
    numElems = length + (long) strlen(STRINGDELIMITER) * (numStrings-1);
    tmpStr = (char *) malloc (sizeof(char)*numElems);
    cstr = tmpStr;
    for (ix = 0; ix < numStrings; ++ix) {
      memcpy (cstr, values[ix], strlen(values[ix])); 
      if (ix != (numStrings - 1)) {
        cstr += strlen(values[ix]);
        memcpy (cstr, STRINGDELIMITER, strlen(STRINGDELIMITER));
        cstr += (int) strlen(STRINGDELIMITER);
      }
    }
    if (!sX(ReadADR64(CDF->fp,CDF->CURattrOffset64,
		      ADR_RECORD,&ADR,
    ...
    if (!WriteAccess64(CDF,FALSE,&pStatus)) return pStatus;
    eOffset = (!zOp?CDF->CURgrEntryOffset64:CDF->CURzEntryOffset64);
    if (eOffset != (OFF_T) RESERVED_ENTRYOFFSET64) {
      /************************************************************************
      * The entry already exists.
      ************************************************************************/
	...
	********************************************************************/
	...
	if (!sX(ResizeIR64(CDF,eOffset,
			   AEDR_BASE_SIZE64+numElems,
			   &newOffset,TRUE,NULL),&pStatus)) {
	  AbortAccess64 (CDF, UPDATE, noDELETE);
	  return pStatus;
	}
	AEDR.RecordSize = AEDR_BASE_SIZE64 + numElems;
	AEDR.DataType = dataType;
	AEDR.NumElems = numElems;
        AEDR.NumStrings = numStrings;
	if (!sX(WriteAEDR64(CDF,CDF->fp,newOffset,
			    AEDR_RECORD,&AEDR,tmpStr,
			    AEDR_NULL),&pStatus)) {
	  AbortAccess64 (CDF, UPDATE, noDELETE);
	  return pStatus;
	}
	...
	********************************************************************/
	...
	if (!sX(WriteAEDR64(CDF,CDF->fp,eOffset,
      ...
      ************************************************************************/
      struct AEDRstruct64 AEDR;
      OFF_T lastOffset;
      zEntry = BOO(!zOp,FALSE, BOO(zModeON(CDF),
		   BOO(CDF->CURzEntryNum < CDF->NrVars,FALSE,TRUE),TRUE));
      entryN = BOO(!zOp,CDF->CURgrEntryNum,
		   BOO(zModeON(CDF), BOO(CDF->CURzEntryNum < CDF->NrVars,
			                 CDF->CURzEntryNum,CDF->CURzEntryNum -
                                                           CDF->NrVars),
		                         CDF->CURzEntryNum));
      AEDR.RecordSize = AEDR_BASE_SIZE64 + numElems;
      AEDR.RecordType = BOO(zEntry,AzEDR_,AgrEDR_);
