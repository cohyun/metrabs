...
{
  long read_only_mode;
  struct AEDRstructExt *CurAEDR = NULL;
  va_list ap; CDFstatus pStatus = CDF_OK;
#if defined(STDARG)
  va_start (ap, offset);
#else
  ...
  for (;;) {
	 ...
	 void *value = va_arg (ap, void *); size_t nBytes;
         if (read_only_mode == READONLYoff)
         {
             ...
             if (!Read32(fp,&(AEDR->NumStrings))) return CRE;
	 ...
	 switch (field) {
	   ...
	   case AEDR_NUMELEMS: {
           ...
           case AEDR_NUMSTRINGS: {
               if (read_only_mode == READONLYoff)
               {
                   tOffset += (OFF_T) AEDR_NUMSTRINGS_OFFSET; break;
               }
               else
               {
                                     ...
                                     Int32 MaxEntry)
{
  ...
  ***************************************************************************/
  while (NxtAEDR != 0 && iEntries != NumEntries)
  {
      ...
      if (!Read32(fp,&(TempAEDR->AEDR.NumStrings))) return CRE;
