...
/*  if (!ValidCDFname(CDFpathT)) return BAD_CDF_NAME; */
    ...
    **************************************************************************/
	...
	return pStatus;
      ...
      if (vType == SPARSE_RECORDS_) {
          if (Var != NULL) {
            if (Var->stage.areaOffset != (Int32) NO_OFFSET &&
        ...
        if (Var != NULL) {
          if (Var->stage.areaOffset != (Int32) NO_OFFSET &&
              Var->stage.firstRec != NO_RECORD &&
              Var->stage.dotOffset == (Int32) NO_OFFSET) {
            Int32 tmp = (Var->stage.lastRec > maxRec ? maxRec :
                         Var->stage.lastRec);
            lastAllocatedRecN += (tmp - Var->stage.firstRec + 1);
          }
  ...
  ****************************************************************************/
  ...
  case zENTRY_NUMSTRINGS_: {
    ...
    if (!sX(CheckEntryOp(CDF,Va->item),&pStatus)) return pStatus;
    eOffset = BOO(dataOp, CDF->CURgrEntryOffset,
                          CDF->CURzEntryOffset);
    if (eOffset == (Int32) RESERVED_ENTRYOFFSET) return NO_SUCH_ENTRY;
    if (!sX(ReadAEDR(CDF->fp,eOffset,
    ...
    *value = (long) BOO((tValue>0), tValue, 0);
    break;
  }
  /****************************************************************************
  ...
  case zENTRY_STRINGSDATA_: {
    ...
    offset = (!zOp?CDF->CURgrEntryOffset:CDF->CURzEntryOffset);
    if (offset == (Int32) RESERVED_ENTRYOFFSET) return NO_SUCH_ENTRY;
    if (!sX(ReadAEDR(CDF->fp,offset,
                     AEDR_DATATYPE,&dataType,
                     AEDR_NUMELEMS,&numElems,
                     AEDR_NULL),&pStatus)) {
