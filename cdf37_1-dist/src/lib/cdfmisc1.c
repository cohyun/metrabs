...
/******************************************************************************
* FindNumberOfItems
* Find the number of items in a string. Each item is delimited by the given
* delimiter string.
******************************************************************************/

VISIBLE_PREFIX long FindNumberOfItems (string, numElems, delimiter)
char *string;
long numElems;
char *delimiter;
{
  long count;
  char *tmp, *tmp2, *tmp3;
  int dlen = (int) strlen(delimiter);
  tmp = malloc ((size_t) numElems+1);
  memcpy (tmp, string, (size_t) numElems);
  *(tmp+numElems) = '\0';
  count = 1;
  tmp2 = tmp;
  tmp3 = strstr(tmp2, delimiter);
  while (tmp3 != NULL) {
    tmp2 = tmp3 + dlen;
    ++count;
    tmp3 = strstr(tmp2, delimiter);
...
/******************************************************************************
* CDF_Free_String
* This is used to free the dynamic space allocated by the CDF library for
* reading the string-based variable attribute entry.
******************************************************************************/

VISIBLE_PREFIX void CDF_Free_String (numStrings, strings)
long numStrings;
char **strings;
{
  int ix;
  for (ix = 0; ix < (int) numStrings; ++ix) free(strings[ix]);
...
/******************************************************************************
* FillNulsToString.
* This function scans for Nul in a string buffer. If found, it fills Nul(s)
* until the end of the number of elements. This happens when a string is
* entered with a length shorter than the defined number of elements. The
* buffer's size is a number of numElms, which could contain multiple strings.
******************************************************************************/
STATICforIDL void FillNulsToString (buffer, totalBytes, numElems)
char  *buffer;
int   totalBytes;
int   numElems;
{
  ...
  for (i = 0; i < l; ++i) {
    for (j = 0; j < numElems; ++j) {
      if (*(buffer+i*numElems+j) == '\0') {
        for (k = j+1; k < numElems; ++k)
          *(buffer+i*numElems+k) = '\0';
        break;
      }
    }
  }
