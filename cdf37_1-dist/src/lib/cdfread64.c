...
{
  long read_only_mode;
  struct AEDRstructExt64 *CurAEDR = NULL;
  va_list ap; CDFstatus pStatus = CDF_OK;
#if defined(STDARG)
  va_start (ap, offset);
#else
  ...
  ***************************************************************************/
  for (;;) {
       ...
       case AEDR_RECORD: {
         ...
         if (read_only_mode == READONLYoff)
         {
             ...
             /*
               ...
               if (!Read32_64(fp,&(AEDR->AEDR.NumStrings))) return CRE;
             ...
             */
             if (value != NULL) {
               ...
               if (STRINGdataType(AEDR->DataType)) {
                 FillNulsToString((char *)value, (int)AEDR->NumElems,
                                  (int)AEDR->NumElems);
               }
             };
         }
         else 
         {
             ...
             AEDR->NumStrings = CurAEDR->AEDR.NumStrings;
	 ...
	 void *value = va_arg (ap, void *);
	     ...
	     if (!READv64(value,nBytes,1,fp)) return CRE;
             if (STRINGdataType(dataType)) {
               FillNulsToString((char *)value, (int)numElems, (int)numElems);
             }
         }
         else
         {
	 ...
	 switch (field) {
	   ...
	   case AEDR_NUMSTRINGS: {
               if (read_only_mode == READONLYoff)
               {
                   tOffset += (OFF_T) AEDR_NUMSTRINGS_OFFSET64; break;
               }
               else
               {
                   *buffer = CurAEDR->AEDR.NumStrings;
               }
               break;
           }
	   default: return CDF_INTERNAL_ERROR;
...
#endif
{
  ...
  for (;;) {
	 ...
	 if (PADvalueBITset(VDR->Flags) && padValue != NULL) {
	   ...
	   if (!READv64(padValue,nBytes,1,fp)) return CRE;
           if (STRINGdataType(VDR->DataType)) {
             FillNulsToString((char *)padValue, (int)VDR->NumElems,
                              (int)VDR->NumElems);
           }
	 }
	 break;
	 ...
	 if (!READv64(padValue,nBytes,1,fp)) return CRE;
         if (STRINGdataType(dataType)) {
           FillNulsToString((char *)padValue, (int)numElems,
                            (int)numElems);
         }
	 break;
       }
       ...
       case VDR_CPRorSPR: {
         ...
         switch (field) {
           case VDR_RECORDSIZE:
             tOffset += (OFF_T) VDR_RECORDSIZE_OFFSET64;
             break; 
  ...
  for (;;) {
	 ...
	 break;
       ...
       case VXR_VXRNEXT: {
         ...
         switch (field) {
           case VXR_RECORDSIZE:
             tOffset += (OFF_T) VXR_RECORDSIZE_OFFSET64;
             break; 
	 ...
	 switch (field) {
	   case VXR_RECORDTYPE:
             tOffset += (OFF_T) VXR_RECORDTYPE_OFFSET64;
             break;
	   case VXR_NENTRIES: tOffset += (OFF_T) VXR_NENTRIES_OFFSET64; break;
	   case VXR_NUSEDENTRIES:
             tOffset += (OFF_T) VXR_NUSEDENTRIES_OFFSET64;
             break;
	   default: return CDF_INTERNAL_ERROR;
	 }
	 if (!SEEKv64(fp,tOffset,vSEEK_SET)) return CRE;
...
#endif
{
  ...
  for (;;) {
	 ...
	 switch (field) {
	   case VVR_RECORDTYPE:
             tOffset += (OFF_T) VVR_RECORDTYPE_OFFSET64;
             break;
	   default: return CDF_INTERNAL_ERROR;
	 }
	 if (!SEEKv64(fp,tOffset,vSEEK_SET)) return CRE;
...
#endif
{
  ...
  for (;;) {
	 ...
	 switch (field) {
	   case UIR_RECORDSIZE:
             tOffset += (OFF_T) UIR_RECORDSIZE_OFFSET64;
             break;
...
#endif
{
  ...
  for (;;) {
	 ...
	 switch (field) {
	   case CCR_RECORDSIZE:
             tOffset += (OFF_T) CCR_RECORDSIZE_OFFSET64;
             break;
...
#endif
{
  ...
  for (;;) {
	 ...
	 switch (field) {
	   case CPR_RECORDTYPE:
             tOffset += (OFF_T) CPR_RECORDTYPE_OFFSET64;
             break;
...
#endif
{
  ...
  for (;;) {
	 ...
	 switch (field) {
	   case SPR_RECORDTYPE:
             tOffset += (OFF_T) SPR_RECORDTYPE_OFFSET64;
             break;
...
#endif
{
  ...
  for (;;) {
	 ...
	 switch (field) {
	   case CVVR_RECORDSIZE:
             tOffset += (OFF_T) CVVR_RECORDSIZE_OFFSET64;
             break;
	 ...
	 Int32 *buffer = va_arg (ap, Int32 *); OFF_T tOffset = offset;
	 switch (field) {
	   case CVVR_RECORDTYPE:
             tOffset += (OFF_T) CVVR_RECORDTYPE_OFFSET64;
             break;
...
STATICforIDL CDFstatus ReadADRList64 (vFILE *fp)
{
  ...
  ***************************************************************************/
  ...
  {
      fp->ADRList64[i] = cdf_AllocateMemory((size_t)sizeof(struct ADRstruct64),
                                            NULL);
                                       ...
                                       Int32 MaxEntry)
{
  ...
  ***************************************************************************/
  ...
  {
      TempAEDR = cdf_AllocateMemory((size_t)sizeof(struct AEDRstructExt64),
                                    NULL);
      ...
      /*
        ...
        if (!Read32_64(fp,&(TempAEDR->AEDR.NumStrings))) return CRE;  
      ...
      */
      ...
      if (STRINGdataType(TempAEDR->AEDR.DataType)) {
        FillNulsToString((char *)(TempAEDR->Value),
